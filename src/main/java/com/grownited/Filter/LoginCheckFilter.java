 ...
 * 
 * @Component public class LoginCheckFilter implements Filter {
 * 
 * ArrayList<String> publicURL = new ArrayList<>();
 * 
 * public LoginCheckFilter() { publicURL.add("/login");
 * publicURL.add("/signup"); publicURL.add("/saveuser");
 * publicURL.add("/forgetpassword"); publicURL.add("/sendotp");
 * publicURL.add("/authenticate"); publicURL.add("/logout");
 ...
 * uri.contains("dist") || uri.contains("images")) { chain.doFilter(request,
 * response);//go ahead } else { HttpSession session = req.getSession();
 * UserEntity user = (UserEntity) session.getAttribute("user");
 * 
 * if (user == null) { req.getRequestDispatcher("login").forward(request,
 * response); }else {
 * 
